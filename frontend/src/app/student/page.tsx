...
export default function StudentPage() {
  ...
  async function requestChat(prompt: string): Promise<Omit<ChatMessage, "id">> {
    if (!token) {
    ...
    const response = await fetch(buildApiUrl("/chat"), {
      ...
      body: JSON.stringify({ message: prompt, k: 5 }),
    });
    ...
    const source = Array.isArray(payload?.sources) ? payload.sources[0] : null;
    const citation: Citation | undefined = source
      ? {
          material: source.filename || "Course material",
          snippet:
            typeof source.chunk_index === "number"
              ? `Referenced chunk ${source.chunk_index}`
              : undefined,
        }
      : undefined;
  ...
  function sendMessage(text: string) {
    ...
    requestChat(prompt)
      ...
      .catch((error) => {
        const fallback = buildAssistantMessage(prompt);
        appendMessage({
          role: "error",
          text:
            error instanceof Error
              ? `${error.message} (showing fallback response)`
              : "Chat failed (showing fallback response)",
        });
